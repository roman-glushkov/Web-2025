...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".post__like").forEach((button) => {
    button.addEventListener("click", function (e) {
      e.preventDefault();
      ...
      fetch("like.php", {
        ...
        .then((data) => {
          if (data.success) {
            ...
            this.querySelector(".post__like-count").textContent = data.count;

            const icon = this.querySelector(".post__like-icon");
            icon.src = data.is_liked
              ? "static/images/heart.png"
              : "static/images/heart.png";

            data.is_liked
              ? this.classList.add("post__like--active")
              : this.classList.remove("post__like--active");
          } else {
            showError("Ошибка при обновлении лайка");
          }
        })
        .catch((error) => {
